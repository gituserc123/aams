	...
	$.parser = {
		...
		parse: function(context){
			...
			for(var i=0; i<$.parser.plugins.length; i++){
				...
				if(name=='validatebox'){
					r = $('.easyui-'+name+',.txt-validate',context);
				}
				if (r.length){
					if (r[name]){
						r.each(function(){
...
(function($){
	...
	function validate(target){
		...
		var needMsg = $(target).attr('noNull');
		if(needMsg){
		...
		rules: {
			...
			remote: {
				validator: function(value, param){
					...
					var urlAp = param[0].split('?');//处理带参数的url，解决ie下文本或者参数为中文兼容问题
					var urlP = urlAp[1];
					if(urlP){
...
(function($){
	...
	function setSize(target, param){
		...
		var icons = addon.find('.textbox-icon:not(".easyui-clearIcon")');
...
(function($){
	...
	function validate(target,noErrFocus){
		if ($.fn.validatebox){
			var t = $(target);
			t.find('.validatebox-text:not(:disabled)').validatebox('validate');
			var invalidbox = t.find('.validatebox-invalid');
			if(!noErrFocus){
				invalidbox.filter(':not(:disabled):first').focus().triggerHandler('mouseenter');
			}
	...
	$.fn.form.methods = {
		...
		validate: function(jq,noErrFocus){
			return validate(jq[0],noErrFocus);
		},
		disableValidation: function(jq){
			return jq.each(function(){
		...
		function getEditorConf(name){
			...
			return {
				...
				setValue: function(target, value){
					var $input = $(target);
					if (isA(target, name)){
						$input = $(target).next().find('input').eq(0);
						var opts = $(target)[name]('options');
						if (opts.multiple){
							if (value){
...
//						msg.css('marginLeft', -msg.outerWidth()/2);
	...
	var defaultView = {
		...
		renderRow: function(target, fields, frozen, rowIndex, rowData){
			...
			var cc = [];
			if (frozen && opts.rownumbers){
				var rownumber = parseInt(rowIndex) + 1;
				if (opts.pagination){
			...
			for(var i=0; i<fields.length; i++){
				...
				if (col){
					...
					if (!col.checkbox){
						if (col.align){styleB += 'text-align:' + col.align + ';'}
						if (!opts.nowrap){
							styleB += 'white-space:normal;height:auto;';
					...
					var tdTxt = col.formatter?col.formatter(value, rowData, rowIndex):value;
					var formatTitle = (value+'').replace(/[\r\n]/g,"&#13;");
					formatTitle = formatTitle.replace(/[\t ]/g,"&nbsp;");
					formatTitle = formatTitle.replace(/</g,"&lt;");
					formatTitle = formatTitle.replace(/>/g,"&gt;");
					formatTitle = formatTitle.replace(/"/g,"&quot;");
					var tdTitle = col.titletip?(' title='+formatTitle+''):'';
					// var tdTitle = ' title='+tdTxt;

					cc.push('<td field="' + field + '" ' + cls + ' ' + style + ' '+tdTitle+'>');
					cc.push('<div style="' + styleB + '" ');
		...
		updateRow: function(target, rowIndex, row){
			...
			function _update(frozen){
				...
				tr[0].innerHTML = this.renderRow.call(this, target, fields, frozen, rowIndex, rowData);
...
//							val = row[field] || '&nbsp;';
		...
		updateRow: function(target, id, row){
			...
			function _update(frozen){
				...
				var checked = tr.find('div.datagrid-cell-check input[type=checkbox]').is(':checked');
				tr[0].innerHTML = this.renderRow(target, fields, frozen, depth, rowData);
...
(function($){
	...
	function inputEventHandler(e){
		...
		switch(e.keyCode){
		case 38:	// up
			if(!e.ctrlKey){opts.keyHandler.up.call(target, e);}
			break;
		case 40:	// down
			if(!e.ctrlKey){opts.keyHandler.down.call(target, e);}
			break;
		case 37:	// left
...
//	}
	...
	function setValues(target, values){
		...
		if(opts.onBeforeChange){//增加 onBeforeChange 方法，return true才可以继续
			var a=values[0],b=oldValues[0];
			if(opts.multiple){
				a =values,b =oldValues;
			}
			if(opts.onBeforeChange.call(target,a,b)===false){
		...
		if (changed){
			if(opts.clearCanSelect&&opts.onSelect&&values.length==1&&values[0]===''){//combobox及comboxgrid清空时 如果clearCanSelect:true 则触发onSelect事件
				if($(target).hasClass('combobox-f')){opts.onSelect.call(target,{});}//返回空数组
				else if($(target).hasClass('combogrid-f')){opts.onSelect.call(target,-1,{});}//返回空数组
			}
...
//				clear(this);
	...
	$.fn.combo.parseOptions = function(target){
		...
		return $.extend({}, $.fn.textbox.parseOptions(target), $.parser.parseOptions(target, [
			...
			{panelWidth:'number',hasDownArrow:'boolean',minRemoteKeyLen:'number',delay:'number',reversed:'boolean',multivalue:'boolean',selectOnNavigation:'boolean'},
	...
	var defaultView = {
		render: function(target, container, data){
			...
			for(var i=0; i<data.length; i++){
				...
				var itemOne = opts.formatter ? opts.formatter.call(target, row) : s;
				...
				dd.push('<div id="' + (state.itemIdPrefix+'_'+i) + '" class="' + cls + '" title="'+itemTxt+'">');
...
(function($){
	...
	function doEnter(target){
		var state = $.data(target, 'combotree');
		state.remainText = false;
		(!state.options.enterFree) && $(target).combotree('setValues', $(target).combotree('getValues'));
