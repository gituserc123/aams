...
requirejs(['pub'],function () {
  $grid.newGrid('#gridBox',{
            url:'${base}/ui/sys/authorize/hospital/lookUpAuth/${staffId!}/${instId!}',
            idField:'id',
            treeField:'name',
            fitColumns : false,
            flatData : true,
            columns:[[
                {title:'模块名称',field:'name',width:'20%',align:'left'},
                {title:'操作权限',field:'power',width:'80%',align:'left',formatter: function (value,row) {
                  var roleRowHtml = '';
                  if(row.power){
