...
<script type="text/javascript">
    requirejs(['pub'],function () {
        $('#ul-powerTree').tree({
            animate : true,
            url : '${base}/ui/sys/authorize/hospital/getRoleTreeByStaffInst/${staffId!}', //根据用户和医院id角色树
            onlyLeafCheck : true,
            flatData:true,
            checkbox : true,
            lines : true
        ...
        $('.btn-submitRole').click(function () {
            ...
            $ajax.post('${base}/ui/sys/authorize/hospital/update',sendData,true).done(function (rst) {
